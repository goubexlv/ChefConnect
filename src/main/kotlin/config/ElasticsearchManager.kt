...
import cm.daccvo.utils.Constants.ELASTIC_APIKEY
...
//    }
    ...
    fun createClient(): ElasticsearchClient {
        ...
        // Convertir l'URL du cloud en host
        val uri = java.net.URI(ELASTIC_HOST)
        val host = HttpHost(uri.host, uri.port.takeIf { it != -1 } ?: 443, uri.scheme)

        val restClient = RestClient.builder(host)
            .setDefaultHeaders(
                arrayOf(
                    BasicHeader(HttpHeaders.AUTHORIZATION, "ApiKey $ELASTIC_APIKEY")
                )
            )
            .build()
